...
Step 3.5: Research before creating — for EACH candidate new article, first check whether it already exists: Grep {{KB_DIR}}/wiki/_index.md for the distinctive words of the candidate title, then Grep the content dirs (wiki/ projects/ research/ solutions/ tools/ decisions/ patterns/) for 2-3 distinctive keywords of the topic (also matches `aliases:` frontmatter). If a matching or overlapping article exists — even from another domain — UPDATE it instead of creating a near-duplicate: append a section, add the cross-reference, extend its `sources:`, and reuse its exact `title:` in wikilinks. Create a new article only when the search comes back empty. A near-duplicate page is worse than no page: it splits future updates across files and corrupts dream's contradiction resolution. This is the core value of the KB.
