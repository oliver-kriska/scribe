...
**How to search:** For natural-language semantic retrieval, prefer the host-launched qmd MCP `query` tool. qmd tools may be deferred/lazy and absent from the initially displayed tool list, so before claiming MCP is unavailable or falling back to the shell, inspect the available/deferred tool catalog for a qmd query tool (commonly `mcp__qmd__query`; in runtimes that expose `ALL_TOOLS`, check it there). Always pass the user's question as `intent`, and call qmd MCP model operations serially because concurrent local-model calls can contend or stall. Results include paths you can read for full context; use the matching MCP `get` tool for a central follow-up document.

Use shell `qmd search "<keywords>"` for exact-term, no-model retrieval. If MCP discovery confirms that qmd `query` is unavailable and semantic retrieval is necessary, run shell `qmd query "<natural language question>"` outside the sandbox with approval; Codex's sandbox cannot reliably initialize qmd's Metal-backed local models. All qmd commands work from any directory because collections use absolute paths, so **never `cd` into {{.KBDir}} first**. For structural navigation once you're inside the KB, read `{{.KBDir}}/wiki/_index.md`.

**When to search proactively — don't wait for {{.OwnerName}} to ask.** The KB is only valuable if it's consulted before decisions, not after. {{.OwnerName}} has spent research effort on every one of the situations below and the answers are on disk; skipping the search wastes that work. Query the KB at the start of any of these, without asking first:
...
**Follow the graph — one hop.** qmd returns flat ranked hits; it doesn't traverse wikilinks. After you read a top result, if it contains a `[[Wikilink]]` or a `related:` frontmatter entry that names a concept **central to the user's question** (not just incidental), fetch that neighbor with the qmd MCP `get` tool (or shell `qmd get "<path-or-docid>"` on the fallback path) before answering. Stop at one hop unless the second hop is clearly needed. This matches the progressive-disclosure-retrieval pattern (L2 = seed articles + 1–2 hops). Central-to-the-question is the bar — don't expand on every wikilink you see, or you'll drift off-topic and bloat context.
